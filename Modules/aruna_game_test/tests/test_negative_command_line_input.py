...
@tagged('aruna', 'test_negative_command_line_input',
...
class TestCommandLineInput(TransactionCase):
    def setUp(self):
        ...
        # TestingException
        self.wizard_model: InputCommandWizard = self.env['input.command.wizard'].with_context(
            is_testing=True)
    ...
    def test_3_error_when_navigating_the_table(self):
        ...
        try:
            ...
            # Check the position before the object would fall, should be
            # 4,0,EAST
            self.assertEqual([4, 0, eObjectFacing.east.name.upper()],
                             err.error_data.get('position_before_error'))
            # Check the position when error is triggered / when the object is
            # falling due to moving forward
            self.assertEqual([5, 0, eObjectFacing.east.name.upper()],
                             err.error_data.get('position_at_error'))
